...
# I learned about feature ablation methods on this platform: https://developers.google.com/machine-learning/glossary  
def ablate_features_and_evaluate(training_features, gold_labels, modelname, inputfile, outputfile):
    """
    Ablate features iteratively and evaluate the models' performance for each subset of features.
    :param training_features: list of training features
    :param gold_labels: list, gold labels
    :param modelname: str, name of the model ('logreg', 'NB', 'SVM')
    :param inputfile: str, path to the input data
    :param outputfile: str, path to save the output file with predictions
    :return: None
    """
    num_features = len(training_features[0])
    
    # Evaluate features iteratively, first taking a single feature, then adding 1 more in each round
    for i in range(1, num_features + 1):
        print(f"\nEvaluating {i} features for {modelname}:")

        # Create a subset of features
        subset_features = [{k: d[k] for k in list(training_features[0].keys())[:i]} for d in training_features]

        # Convert the subset to a numpy array
        feature_matrix = np.array([list(d.values()) for d in subset_features])

        # train the classifier
        ml_model, vec = create_classifier(subset_features, gold_labels, modelname)

        # Classify data and evaluate the model
        feature_names = ', '.join(list(training_features[0].keys())[:i])
        print(f"Evaluated features: {feature_names}")
        outputfile_subset = outputfile.replace('.conll', f'.{modelname}_{i}features.conll')
        classify_data(ml_model, vec, inputfile, outputfile_subset)
        evaluate_model(outputfile_subset, ml_model)
...
def main(argv=None):
    ...
    feature_matrix = np.array([list(d.values()) for d in training_features])

    for modelname in ['logreg', 'NB', 'SVM']:
        ablate_features_and_evaluate(training_features, gold_labels, modelname, inputfile, outputfile)
